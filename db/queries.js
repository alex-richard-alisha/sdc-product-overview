...
const getStyles = (productID, callback) => {
  const query = [
    ...
    'jsonb_agg(distinct jsonb_build_object(\'thumbnail_url\', photos.thumbnail_url, \'url\', photos.url)) AS photos, ', 'jsonb_object_agg(skus.sku, jsonb_build_object(\'quantity\', skus.quantity, \'size\', skus.size)) AS skus ',
